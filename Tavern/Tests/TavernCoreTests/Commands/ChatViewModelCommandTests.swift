...
@Suite("ChatViewModel Slash Command Integration Tests", .timeLimit(.minutes(2)))
struct ChatViewModelCommandTests {
    ...
    @Test("Slash command is intercepted and not sent to servitor")
    @MainActor
    func slashCommandIntercepted() async {
        let mock = MockServitor(name: "TestServitor", responses: ["should not appear"])
        ...
        // Servitor should NOT have been called
    ...
    func slashCommandErrorShowsInChat() async {
        let mock = MockServitor(name: "TestServitor")
        let viewModel = ChatViewModel(servitor: mock, loadHistory: false)
    ...
    @Test("Regular text passes through to servitor when dispatcher is set")
    @MainActor
    func regularTextPassesThrough() async {
        let mock = MockServitor(name: "TestServitor", responses: ["servitor response"])
        let viewModel = ChatViewModel(servitor: mock, loadHistory: false)
        ...
        // Servitor SHOULD have been called
        #expect(mock.sendCalls == ["hello world"])
        #expect(viewModel.messages.count == 2)
    }

    @Test("Slash command without dispatcher passes through to servitor")
    @MainActor
    func slashCommandWithoutDispatcherPassesThrough() async {
        ...
        // Without dispatcher, /compact goes to servitor as regular text
