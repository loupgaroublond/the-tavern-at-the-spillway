...
**Problem:** The project is in `~/Documents` which is iCloud-synced. When `xcodegen generate` runs while iCloud is syncing (or Xcode has the project open), iCloud detects a "conflict" and creates numbered copies like `Tavern 5.xcodeproj`, `Tavern 6.xcodeproj`, etc. The original often gets corrupted (missing `project.pbxproj`).

**Root cause:** `brctl status` shows the entire project, including `.build/` and `.git/`, is being actively synced. The `com.apple.fileprovider.fpfs#P` extended attributes on the numbered copies confirm iCloud created them.

**Workaround:**
```bash
# Clean up corrupted copies and regenerate
cd Tavern
...
1. Move project to a non-synced location (e.g., `~/Developer/`, `~/.local/projects/`)
2. Exclude `.xcodeproj` from iCloud sync (rename to `.xcodeproj.nosync` and symlink, or use `xattr`)
3. Add `.build/` and `.xcodeproj/` to `.nosync` extension pattern

**Note:** Derived data is already built to `~/.local/builds/tavern` to avoid iCloud interference with code signing. The xcodeproj itself is the remaining problem.
