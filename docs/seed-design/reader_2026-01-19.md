...
## 6. Sandbox and Isolation (from initial_notes.md walkthrough)

This section was refined during a line-by-line walkthrough of the seed notes.
...
## 7. Core Systems (from initial_notes.md walkthrough)

These systems form the backbone of the framework.
...
**Orthogonal to task modes:** Agent may work on just one part of a workflow. Task modes (execute, delegate, plan, break up, unify) are about agent runtime, not workflow state.
...
- Check that agents are not off course
- Token budgets — cap spending per task/agent

Telemetry for LLMs is still an open topic generally.


### Gang of Experts

Specialized agents (reviewer, tester, architect, etc.) available on demand. User can customize and change them.


### Meta Process

Decision layer for selecting workflows/tools for a given task. Goal is improving overall performance — not just picking, but learning what works best.


### Creative Meta Process

Measure workflows, improve them over time. One aspect of the broader self-improvement goal (?3). Other aspects TBD.


### Discovery Sharing

Goal: agents (even drones) notice oddities while working and share them.

**Challenge:** No deterministic hook to enforce "did you notice anything?" — can't checkpoint for curiosity.

**Solution:** Prompt engineering. Invite agents to message about discoveries even while continuing their main task. Flow: agent notices anomaly → writes message → overseer responds or escalates.


---

## 8. UI Concepts (from initial_notes.md walkthrough)

### Frontend as New IDE Type

The UI is more than a chat interface:

- Shows document store AND whole filesystem
- File names are clickable links (solves terminal problem)
- Organized for managing many agents (solves IDE problem)
...
Live view of what's modified in the changeset/overlay. Separate from "asked vs done" comparison (which belongs in status cards).
...
- Agents queue up in order
- Each agent sees what's ahead of them
- Refine against a predictable target (queue in front of you)
- Serialized merges = fewer conflicts
- Not a race/melee to merge first


### Question Triage

Classify questions as quick vs deep. Notifications convey complexity so user knows what they're getting into before opening a chat.


### Session Inbox

Dashboard for agent conversations:

- Unread
- Needs attention
- Done but needs closure (user must explicitly close the loop)


### Todo + Session Unified

Agent session IS a todo item. Todo items can become/spawn sessions. Covered in User Flow — tasks appear as both todo items AND chat interfaces.
...
Go back to checkpoint, tweak prompt, fork new timeline. Works at both agent level and changeset level.


---

## 9. Agent Communication (from initial_notes.md walkthrough)
...
**Resolution:** Do not use git worktrees. Use overlay filesystems orthogonal to git. Changesets work across multiple repos. See "Five Sandbox Primitives" above.
...
**Context:** User needs more than hooks — possibly a full workflow DSL or state machine. Discussion deferred — will address later.
...
**Partial resolution:** Five sandbox primitives defined (Changeset, Platform, Isolation, Outputs, Software). Mix-and-match approach per agent. See "Five Sandbox Primitives" above.
...
**File:** `transcript_2026-01-19-1144.md` (later sections)
...
### Session 3 — initial_notes.md Walkthrough (later on 01-19)

**File:** `transcript_2026-01-19-1144.md` (lines 751-1332)

Line-by-line discussion of seed ideas in `initial_notes.md`. Key topics covered:

| Line | Topic | Resolution |
|------|-------|------------|
| 1 | Preflight checks | Spawn-time verification (sandbox, resources, credentials). Fail = no start + parent notified. |
| 2 | Sandbox primitives | Five primitives: Changeset, Platform, Isolation, Outputs, Software. Mix and match per agent. **Resolves ?1.** |
| 4 | Document store | FOUNDATION concept — memory backbone, messaging, work queues. Authoritative source. |
| 5 | Project space | = project root directory (original or overlay) |
| 6 | Workflow engine | State machine, track/enforce steps, manage templates. Composability TBD. |
| 7 | Spec engine | Verify output matches spec, detect conflicts, check completeness. Vague/TBD. |
| 8 | Starter workflows | Rule of 5 (5 passes), verification layers (lint → structure → arch → perf). |
| 9 | Efficiency | Check agents on course, token budgets. |
| 10 | Gang of experts | Specialized agents on demand, user can customize. |
| 11 | Meta process | Decision layer for selecting workflows/tools. |
| 12 | Creative meta process | Measure workflows, improve them. One aspect of self-improvement. |
| 13 | Discovery sharing | Prompt agents to notice/share oddities. Can't enforce deterministically. |
| 14 | Frontend | New IDE type — doc store + filesystem, clickable links, conversation mgmt + project exploration. |
| 15 | Sessions dashboard | Covered in User Flow. |
| 16 | Context cards | Agent name + assignment + current status for quick context switching. |
| 17 | Fish or cut bait | Triggers: budget exceeded, wrong changeset, spinning, wrong path. Kill + restart. |
| 18 | Rewind and branch | Checkpoint → tweak prompt → fork timeline. Agent + changeset level. |
| 19 | Active diff | Live view of changeset/overlay modifications. |
| 20 | Merge queue | Agents queue up, see what's ahead, refine against predictable target. |
| 21 | Telemetry | Handwavy — surface bottlenecks, faults, stale agents. Details TBD. |
| 22 | Hygiene systems | Cleanup daemons for dead agents, old changesets, workflows, documents. |
| 23 | Surfacing questions | Flexible notification modes — chat popup vs periodic summary. |
| 24 | Zoom decision | Talk high-level vs drill into specific agent. Covered in User Flow. |
| 25 | Sandbox granularity | Covered in line 2 — parent picks at spawn. |
| 26 | Question triage | Quick vs deep. Notifications convey complexity. |
| 27 | Session inbox | Unread, needs attention, done-needs-closure states. |
| 28 | Todo + session unified | Agent session IS a todo item. |
| 29 | Hooks | Mooted — we have direct control via SDK. |
| 30-32 | Agent types | Daemon, mortal, Jake at top. Covered in User Flow. |
| 33 | On-demand scaling | Covered in scaling discussion. |
| 34 | Agent tree view | Dashboard shows tree/list, can summarize. |
| 35 | Message protocol | Certain messages require certain responses, else malfunction signal. |
| 36 | Lateral collaboration | Messaging not just upward, also between agents. |
...
| Preflight checks | Walkthrough, line 1 | Spawn-time verification, fail = no start |
| Five sandbox primitives | Walkthrough, line 2 | Changeset, Platform, Isolation, Outputs, Software — orthogonal to git |
| Document store is foundation | Walkthrough, line 4 | Memory backbone, messaging, work queues, authoritative source |
| Workflow engine = state machine | Walkthrough, line 6 | Track/enforce steps, manage templates |
| Meta process for workflow selection | Walkthrough, line 11 | Decision layer for tools/workflows |
| Discovery sharing via prompts | Walkthrough, line 13 | Cannot enforce deterministically, prompt engineering |
| Frontend = new IDE type | Walkthrough, line 14 | Doc store + filesystem + conversations |
| Message protocol with expectations | Walkthrough, line 35 | Wrong response = malfunction signal |
| Lateral agent collaboration | Walkthrough, line 36 | Messaging between siblings, not just upward |
