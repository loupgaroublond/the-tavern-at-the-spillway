# Product Requirements Document: The Tavern at the Spillway
...
The Tavern at the Spillway is a multi-agent orchestration framework built on the Claude Agent SDK. It enables users to manage complex software development tasks through a hierarchy of AI agents, with Jake (The Proprietor) as the always-available top-level coordinator.
...
**The doc store is everything.** One file per node.
...
Principle: If it needs to persist or be shared, it goes in the doc store.
...
- **Primary Language:** Swift (unless strong reason for TS/Python for low-level layer)
- **GUI:** Swift/SwiftUI (Mac app primary)
...
| Doc store vs filesystem boundary | UI implementation detail |
...
**1. Agent SDK Language Bridge**
- Claude Agent SDK is TypeScript/Python
- PRD says Swift for primary language
- Options: Swift GUI calls TS/Python runtime? Swift bindings? Pure TS/Python backend with Swift frontend?

**2. Jake's Startup**
- What starts Jake? App launch? System service?
- v1 assumption: Jake starts when app opens, stops when app closes

**3. Changeset Implementation**
- Real overlay FS (OverlayFS, APFS)?
- Simulated with file copies?
- Git-based (stash/worktree)?
- v1 might start simpler

**4. API Keys / Auth**
- Where do Claude API keys live?
- User-level config? Per-project? Environment variables?

**5. Session Persistence**
- If app closes mid-task, do sessions survive?
- v1 assumption: ephemeral, sessions die with app?

**6. Doc Store Location**
- Part of project directory (`.tavern/`)?
- Separate user-level location?
- Both (project-specific + global)?
