...
See `../0-transcripts/vocab_naming-themes_2026-01-19-1144.md` for full list.


## 8. Progressive Unlocks

The Tavern reveals content progressively based on user engagement. This rewards continued use and creates discovery moments.


### 8.1 Concept

Track the number of messages a user has sent. As the count grows, unlock additional content:
...
The unlock experience should feel organic, not gamified. Jake doesn't announce unlocks with fanfare — he just starts using new words, new themes become available, new jokes appear. Subtle hints only.


### 8.2 Open Questions

These details are intentionally deferred:
...
The core principle is established: engagement unlocks content, feedback is subtle. Implementation details can be refined during development.
...
**See:** `../3-adr/stress-testing.md` for detailed test descriptions and baseline numbers.

### 19.4 Completeness and Correctness Standards

Anyone can make mistakes — human and AI alike. Regressions happen during refactoring. Features get partially implemented and forgotten. Requirements drift from their specification. The development process must be a prophylactic against these errors, not rely on memory or good intentions.

Every feature must be verifiably complete and correct against its specification. Code and tests must carry machine-readable provenance linking them to the requirements they satisfy. Tooling must be able to audit completeness (are all requirements implemented?) and detect regressions (did a change break a requirement's implementation?). If Invariant #3 says agent commitments are verified independently, then development commitments — the chain from requirements to code — must be held to the same verification standard. The process that builds the system is subject to the same rigor as the system itself.
